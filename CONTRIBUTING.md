# Contributing to QuickBooks Desktop Browser Automation
...
- **Broken selectors**: QuickBooks Desktop updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real QuickBooks Desktop instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.QUICKBOOKS_DESKTOP_URL}/path/to/section`);
...
When you find working selectors for QuickBooks Desktop, document them:

```javascript
// QuickBooks Desktop selectors (verified YYYY-MM-DD)
...
QuickBooks Desktop updates its UI periodically. When selectors break:
