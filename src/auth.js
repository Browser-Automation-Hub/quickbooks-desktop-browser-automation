/**
 * auth.js — Authentication handler for QuickBooks Desktop
 * Supports: SSO (Intuit Account / Google), MFA (Intuit MFA / SMS / TOTP)
 *
...
const BASE_URL = process.env.QUICKBOOKS_DESKTOP_URL || 'https://qbo.intuit.com/app/login';
const USERNAME = process.env.QUICKBOOKS_DESKTOP_USERNAME;
const PASSWORD = process.env.QUICKBOOKS_DESKTOP_PASSWORD;
...
async function performLogin(page) {
  if (!USERNAME || !PASSWORD) {
    throw new Error(`Set QUICKBOOKS_DESKTOP_USERNAME and QUICKBOOKS_DESKTOP_PASSWORD in .env`);
  }

  console.log('Logging in to QuickBooks Desktop...');
  await page.goto(BASE_URL, { waitUntil: 'networkidle2' });
