/**
 * session.js — Cookie & localStorage session persistence for QuickBooks Desktop
...
async function loadSession(page) {
  ...
  if (session.localStorage && Object.keys(session.localStorage).length > 0) {
    ...
    const url = new URL(session.url || process.env.QUICKBOOKS_DESKTOP_URL || 'https://localhost');
